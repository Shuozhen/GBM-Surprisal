...
# Define UI
ui <- fluidPage(
  titlePanel("Select a Region Using Metadata Coordinates"),
  sidebarLayout(
    sidebarPanel(
      h4("Selected Points"),
    ...
    mainPanel(
      plotOutput("spatial_plot", brush = brushOpts(id = "plot_brush")) # Enable brushing
    )
  )
)
...
server <- function(input, output) {
  output$spatial_plot <- renderPlot({
    ...
    ggplot(data = coords, aes(x = x, y = y, color = clusters)) +
      geom_point(size = 2) +
      theme_minimal()
  })
  ...
  output$selected_points <- renderPrint({
    if (!is.null(input$plot_brush)) {
      ...
      if (nrow(selected_data) > 0) {
        return(selected_data[, c("x", "y", "seurat_clusters")])
      } else {
...
#P1 Core boundary fixing
# 1) Convert coords to data frame
orig_coords <- samp@images$GBM1027C@boundaries$centroids@coords
...
samp@images$GBM1027C@boundaries$centroids@coords <- coords_new



SpatialDimPlot(samp)

saveRDS(samp, file = "P1_Core_20250409.rds")
